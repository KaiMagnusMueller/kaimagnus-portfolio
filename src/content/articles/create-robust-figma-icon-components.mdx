...
Community files are a great source of icons. In our team, we either search to see if there is already a file of the set that we want to use available, or we build our own libraries with Icon Library Manager. I'm currently working on a post on how we do that.
