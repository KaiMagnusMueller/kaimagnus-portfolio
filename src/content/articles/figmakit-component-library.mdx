...
tags:
    ...
    - Plugin Development
ogImage: ''
description: Learn how I built FigmaKit, a Svelte component library that replicates Figma's native UI, and how it powers my plugin projects.
---

import Figure from '@components/Figure.astro';

FigmaKit is a modernized and extended version of [figma-plugin-ds-svelte](https://github.com/thomas-lowry/figma-plugin-ds-svelte), a library created by Thomas Lowry (designer advocate at Figma), which I've used in plugins like [Tree Navigator](using-the-tree-navigator-plugin) before.
...
If you've used Figma plugins before, you've probably noticed how inconsistent their interfaces can be. Some plugins feel native and integrated, some look decent with minor inconsistencies, while others feel like they were built with random components and dropped into Figma. To improve this aesthetic inconsistency, and to provide a baseline of basic functionality I started building this library.

My goals for this library were: a look and feel close to Figma's native UI3 and lightweight but complete components, using modern Svelte 5.

## Building FigmaKit

FigmaKit uses a very similar prop structure as the original library, so many components are drop-in replacements. However, some components have been extended or modified for consistency with Figma.
...
This could also be handled in a more declarative way with Svelte's `{#snippet}` blocks and separate components for items, groups and so on, but for now I opted for a configuration prop approach, which I feel is simpler.
