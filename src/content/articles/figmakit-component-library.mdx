...
So for this library, a look and feel that is as close to Figma's native UI as possible was especially important.

When I started building ContentKit, people started using UI3 more often, so I initially used basic HTML elements and a small stylesheet to make it look like Figma's new UI. While that worked in the beginning for testing basic functionality, ContentKit quickly became too complex and required more sophisticated UI elements that weren't possible with CSS alone. This, combined with the fact that I wanted to build two things—a web app and plugin—on top of the same Svelte 5 base, led me to take this library and use it as a starting point for my own library based on Svelte 5 and UI3.
...
The `SelectMenu` has been replaced with the more powerful `MultiMenu`, which now supports grouping and nesting of options. And there are a few new components, like the `ToggleButton` and `Dialog`.

The library was built with the help of Claude Sonnet 3.5, which didn't fully support Svelte 5 at the time but did an excellent job with a rough port of the components.
...
```swift
Menu("Text Options") {
    Button("Left Align") { /* action */ }
    Button("Center Align") { /* action */ }
    Menu("Advanced Options") {
        Button("Letter Spacing") { /* action */ }
        Button("Line Height") { /* action */ }
    }
}
```
...
Now with the release of version 2.0, it has become a stable foundation for multiple of my plugin projects. It is modern, lightweight, and consistent. Whether you're building your first Figma plugin or want to integrate it into an existing one, FigmaKit provides all the components you need to create complex interfaces that feel truly native to Figma.
