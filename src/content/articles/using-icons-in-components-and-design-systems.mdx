...
Community files are a great source of icons. In our team we either search if there is already a file of the set that we want to use in the Figma community, or we build our own libraries with Icon Library Manager. If you're interested, you can read about how to import and update your own libraries in [the next post here](https://www.kaimagnus.de/articles/building-icon-libraries-with-icon-library-manager). We then publish them as team libraries, so all projects and files have access to the same always up to date icons.
