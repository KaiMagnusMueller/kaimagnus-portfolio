...
For the past few years I worked as a designer on various applications and projects and been a maintainer and author of internal design systems. Over the years I've spent a lot of time finding ways to build robust components and improving 'designer experience' and usability.  A recurring topic has been the handling of icons and in this post I want to show how we use icons in our design system.

This post assumes basic knowledge of components, instances and libraries in Figma. I recommend visiting Figma help and the official tutorial files as an introduction.

As great as Figma is, it is not without it's issues and from my experience, icons are an area where many aspects come together in a very condensed fashion. Community files are a great source of icons and I've used them as team libraries many times. For the most part this works pretty well, but there are a few things to keep in mind that I want to highlight here. 

{/* ## Naive Approach: Naked Icon Components */}



Imagine you want to create a button component. A very simple approach would be to create a label text and insert an icon from your library directly. This works pretty well. You, the design system maintainer, can change its color to create secondary, and tertiary variants according to your style guide. Designers using your component can select the icon, swap it with other icons from the library and select different variants. In the screenshot below, I created a simple primary button with a white label and icon. What could go wrong?
...
To make the component a bit more nice to use, I like to create an instance swap property for the icon component inside. So designers in your team can select another icon right from the placeholder component instance. Like with all components, Figma displays a list with all components in the same file as the selected one. So if you're using a separate file or downloaded one from the community, it will display all icons you have available. This even works with icons team libraries, very convenient.
