...
## Naive Approach: Raw Icon Components
...
This issue is sometimes hard to spot. In the [IBM Carbon icon file that I maintain](https://www.figma.com/community/file/1141743458996921050/ibm-carbon-icons) for example, all icons are converted to outline strokes and then joined together into  union groups. There are always some that can't be completely reduced to one element so the issue will eventually pop up. 
...
The intersect technique also has the same problem as mentioned earlier, if there are more paths after swapping an icon than before the color will not be applied and it even has a few other issues.[^2] You can experiment with it in the playground file below, where I've included both methods. 
...
- Instance properties don't properly show other icons from the library to swap
- If the icon color was changed, swapping components will sometimes not preserve the color 
