...
One of the biggest pain points when working with Figma Community files is keeping them up to date. There is currently no way to update them like a library or merging new changes as if they were on a branch. But especially with icon sets that are in development, there is a need for a robust update process without having to redownload the file each time.

This is why I made Icon Library Manager (or ILM). It takes a local folder of SVGs and imports them in Figma as components. It preserves the folder structure in the component name for easy filtering and flattens the SVGs as much as possible. [See my last post](https://www.kaimagnus.de/articles/using-icons-in-figma-components) where I go into detail on why this is important.
...
The source code is available on GitHub https://github.com/KaiMagnusMueller/SVG-Icon-Manager. Let me know on [Threads](https://www.threads.net/@ofcoursekai) what you think. I'd be interested in your ideas and suggestions and how you are using ILM.
