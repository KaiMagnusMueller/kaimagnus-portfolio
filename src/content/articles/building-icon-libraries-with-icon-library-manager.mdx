...
title: 'Build and Update Icon Libraries With ILM'
featured:
    - noteworthy
draft: false
tags:
...
<Figure
    ...
    type="video"
    subtitleFileSrc="/subtitles/240511-ilm-demo.vtt"
    aspectRatio="16/9"
    fullWidth></Figure>

This post will show you the process of creating and updating an icon library with ILM. Knowledge of [Node.js/npm](https://nodejs.org/en), the [(macOS) Terminal](https://support.apple.com/de-de/guide/terminal/welcome/mac), Figma components, libraries and general web tech is useful, but not necessary.
...
This is especially helpful when you download icon files that were created with this plugin, like the [file I created for the carbon icon set](https://www.figma.com/community/file/1141743458996921050). After you download it, you can keep it up to date yourself, which would otherwise not be possible in Figma.
...
[^2]: Using icons in designs directly is a bad idea. As I explained [in my previous post](/articles/create-robust-figma-icon-components), overrides are not carried over if the number of layers increases while swapping an icon. This can be prevented by using a placeholder component.

[^3]:
    Figma uses an internal ID for each element to identify overrides, component updates and branches. Because of this, we can't simply delete the component and add it with the updated icon and the same name again. Doing this would cause it to show up during a library update as two separate components, one 'removed' and one 'added'.
