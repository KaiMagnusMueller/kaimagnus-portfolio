...
Aside from realistic data, a big part of prototypes are component properties. From simple button states to variants of cards, list elements notifications, I don't need to tell you they are an essential part of modern UI design. Here however, I saw significant room for improvement. Most content plugins focus on text and few support properties. Even fewer support the many edge cases when trying to update both.[^1] 
...
To handle permissions, billing and to let users edit and share data outside Figma, I created a web app as a companion with almost complete feature parity. 

<Figure 
    alt="Plugin and web app share most features. Components like the table use separate stylesheets, similar to AG Grid, while sharing the same code." 
...
The plugin and web app is built in Svelte, where I could draw from my experience working on [Collections](https://collections.intersectionslab.com/), which is also Svelte based. This allowed me to share some components like the table or dropdown positioning logic between the two parts. 
