...
title: A Grid Component System with DX in Mind
tags:
    - Design System
    - Components
    - Figma
...
Similarly, 'designer experience' – or DX – in the context of component libraries stands for ergonomic components that follow a designer's mental model. They should not get in the way when designing or need detaching. For good DX, a balance between helpful presets in the form of component properties and flexibility via slot components and nested components is important. 
...
Content can be easily swapped via the instance property and thanks to nested properties, things like the state of checkboxes can be set from the main component. Consistent layer names help Figma apply overrides, even when swapping for example from plain text content to a dropdown. This creates a flexible and extensible setup. Which is needed, since AG Grid is also highly customizable and applications display all kinds of data. Designers can use the included content components, but custom spark-lines and other content is very easy to build. 
